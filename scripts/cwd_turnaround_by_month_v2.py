...
def build_monthly_summary(
    ...
    monthly = (
        ...
        .agg(
            ...
            P10_DAYS=(TURNAROUND_FIELD, lambda values: values.quantile(0.10)),
            Q1_DAYS=(TURNAROUND_FIELD, lambda values: values.quantile(0.25)),
            Q3_DAYS=(TURNAROUND_FIELD, lambda values: values.quantile(0.75)),
            P90_DAYS=(TURNAROUND_FIELD, lambda values: values.quantile(0.90)),
...
def build_overall_summary(
    ...
    rows = [
        ...
        {
            "Metric": "10th percentile turnaround (days)",
            "Value": cleaned_df[TURNAROUND_FIELD].quantile(0.10),
            "Notes": "After IQR removal",
        },
        {
            "Metric": "90th percentile turnaround (days)",
            "Value": cleaned_df[TURNAROUND_FIELD].quantile(0.90),
            "Notes": "After IQR removal",
        },
        {
...
def create_summary_workbook(
    ...
    monthly_export = monthly_df[
        ...
        columns={
            "MONTH": "Month",
            "MEDIAN_DAYS": "Median days (primary line)",
            "AVERAGE_DAYS": "Mean days",
            "P10_DAYS": "10th percentile days",
            "Q1_DAYS": "Q1 days",
            "Q3_DAYS": "Q3 days",
    ...
    with pd.ExcelWriter(
        ...
        style_excel_sheet(
            ...
            number_formats={
                "Month": "mmmm yyyy",
                "Median days (primary line)": "0.0",
                "Mean days": "0.0",
                "10th percentile days": "0.0",
                "Q1 days": "0.0",
                "Q3 days": "0.0",
                "90th percentile days": "0.0",
                "Minimum days": "0.0",
                "Maximum days": "0.0",
...
def create_chart(
    ...
    """Create a monthly CWD turnaround-time distribution chart."""

    overall_median = cleaned_df[TURNAROUND_FIELD].median()
    ...
    fig.add_trace(
        go.Scatter(
            x=monthly_df["MONTH"],
            y=monthly_df["P90_DAYS"],
            mode="lines",
            line={"width": 0},
            fill="tonexty",
            fillcolor="rgba(20, 158, 206, 0.10)",
    ...
    fig.add_trace(
        go.Scatter(
            x=monthly_df["MONTH"],
            y=monthly_df["Q3_DAYS"],
            mode="lines",
            line={"width": 0},
            fill="tonexty",
            fillcolor="rgba(20, 158, 206, 0.24)",
            name="Middle 50%",
            legendrank=4,
            hoverinfo="skip",
            connectgaps=False,
    ...
    # -------------------------------------------------------------------------
    # Monthly 90th percentile and mean
    # -------------------------------------------------------------------------
    ...
    # -------------------------------------------------------------------------
    # Monthly median and unified hover details
    # -------------------------------------------------------------------------
    ...
    fig.add_trace(
        go.Scatter(
            ...
            hovertemplate=(
                "<b>%{customdata[0]}</b><br>"
                "Median: %{y:.1f} days<br>"
                "Mean: %{customdata[1]:.1f} days<br>"
                "Middle 50%: %{customdata[3]:.1f}–"
                "%{customdata[4]:.1f} days<br>"
                "10th–90th percentile: %{customdata[2]:.1f}–"
                "%{customdata[5]:.1f} days<br>"
                "90% returned within: %{customdata[5]:.1f} days<br>"
                "Minimum–maximum: %{customdata[6]:.1f}–"
                "%{customdata[7]:.1f} days<br>"
                "Results included: %{customdata[8]:,.0f}<br>"
                "IQR outliers removed: %{customdata[9]:,.0f}"
                "<extra></extra>"
            ),
            connectgaps=False,
        )
    ...
    fig.update_layout(
        title={
            "text": (
                "<b>CWD Test Return Time by Month</b>"
                "<br>"
                "<span style='font-size:12px; color:#666666;'>"
                "Median and percentile ranges after IQR outlier removal"
                "</span>"
            ),
            "x": 0.01,
            "xanchor": "left",
    ...
    fig.add_annotation(
        text=f"Overall median: {overall_median:.1f} days",
        xref="paper",
        yref="y",
        x=1,
        y=overall_median,
    ...
    fig.add_annotation(
        text=f"{total_results:,} results after IQR removal",
        xref="paper",
        yref="paper",
        x=1,
        y=1.10,
