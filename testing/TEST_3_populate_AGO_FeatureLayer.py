# # TESTING: NOT WORKING!     IN PROGRESS - needs further testing and debugging!
# Create an empty ArcGIS Online Hosted Feature Layer using field definitions from an xls Data Dictionary
# The field definitions are read from the Data Dictionary Excel file stored in Object Storage.
# AND populate the template with records from an xls records stored in Object Storage.
...
# 2025-12-16:  Tested loading records into hosted feature layer - NOT WORKING YET.  Issues with mismatched field types??  (salees)
#              Needs further testing and debugging!
...
def delete_existing_item(gis, title, item_type):
    ...
    if not search_results:
        logging.info(f"No existing {item_type} found with title: {title}")
    else:
        logging.info(f"Found {len(search_results)} existing {item_type}(s) with title: {title}")
...
def get_os_xls(s3_client, bucket_name):
    """
    Fetches the latest master Excel file from S3 and returns its DataFrame.
    """
    logging.info('Fetching latest Master XLS from Object Storage')

    #prefix= 'master_dataset/'
    #filepathname = 'master_dataset/cwd_master_dataset_sampling_w_survey_results.xlsx'
    filepathname = 'master_dataset/cwd_master_dataset_sampling_w_survey_results_test.xlsx'   # Test with a subset of data (may need to create file first)
    ...
    latest_file_key = filepathname  #use specific file
    obj = s3_client.get_object(Bucket=bucket_name, Key=latest_file_key)
    data = obj['Body'].read()
    excel_path = pd.ExcelFile(BytesIO(data))

    df_xls = pd.read_excel(excel_path)

    return excel_path, df_xls
...
if __name__ == "__main__":
    ...
    excel_path, df_xls = get_os_xls(s3_client, bucket_name)

    # Configuration
    item_title = "JUNK_FL_Test"   #"CWD_Master_dataset_Template"     #"CWD_FL_Test" (all), "CWD_Public_Table_Test" (public), "CWD_Public_Test_Results_Test" (test_results)
    ...
    file_cleanup = 'y'  # TOGGLE y/n as needed.
    if file_cleanup == 'y':

        logging.info(f"\nAttempting to delete existing items with title: {item_title}")
        #delete_existing_item(gis, item_title, item_type="Microsoft Excel")
        #delete_existing_item(gis, hosted_service_name, item_type="Service Definition")
    ...
    if create_type == "feature_layer" or create_type == "table":
        ...
        if search_results:
            for item in search_results:
                ...
                xls_item.update({}, data=excel_path)
    ...
    published_flag = None  #Default - item does not exist.  Sets to 'True' if it is found below.
